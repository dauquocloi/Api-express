...
const notificationQueue = new Queue(NOTIFICATIONS, redisDb.opts);

notificationQueue.process(5, async (job) => {
	const { data } = job;
	const { notiType } = data;
	try {
		console.log(`[⚙️ Processing] ${notiType} #${job.id}`);
		...
		console.error(`[ Failed] ${notiType} #${job.id}`, error);
		throw error;
	}
});
