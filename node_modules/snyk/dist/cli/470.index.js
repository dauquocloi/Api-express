...
async function testEcosystem(ecosystem, paths, options) {
    ...
    spinner_1.spinner.clearAll();
    if ((0, common_2.isUnmanagedEcosystem)(ecosystem) && options['print-graph']) {
        const [target] = paths;
...
async function formatUnmanagedResults(results, target) {
    ...
    return types_1.TestCommandResult.createJsonTestCommandResult((0, common_1.depGraphToOutputString)(depGraph, target));
}
...
function detectProjectTypeFromFile(file) {
    try {
        const packageManager = (0, detect_1.detectPackageManagerFromFile)(file);
...
async function getDepsFromPlugin(root, options) {
    if (Object.keys(multiProjectProcessors).some((key) => options[key])) {
        ...
        const { files: targetFiles, allFilesFound } = await (0, find_files_1.find)(root, ignore, multiProjectProcessors[scanType].files, levelsDeep);
        ...
        const analyticData = {
            ...
            targetFiles,
            packageManagers: targetFiles.map((file) => (0, detect_1.detectPackageManagerFromFile)(file)),
            levelsDeep,
            ignore,
        };
    ...
    if (!options.scanAllUnmanaged) {
        options.file = options.file || (0, detect_1.detectPackageFile)(root);
    }
...
async function getExtraProjectCount(root, options, inspectResult) {
    ...
    try {
        const { files: extraTargetFiles } = await (0, find_files_1.find)(root, [], detect_1.AUTO_DETECTABLE_FILES);
...
async function getMultiPluginResult(root, options, targetFiles) {
    ...
    for (const targetFile of unprocessedFiles) {
        ...
        optionsClone.packageManager = (0, detect_1.detectPackageManagerFromFile)(pathLib.basename(targetFile));
        try {
...
async function processYarnWorkspacesProjects(root, options, targetFiles) {
    try {
        const { scannedProjects } = await (0, yarn_workspaces_parser_1.processYarnWorkspaces)(root, {
...
async function processNpmWorkspacesProjects(root, options, targetFiles) {
    try {
        const { scannedProjects } = await (0, npm_workspaces_parser_1.processNpmWorkspaces)(root, {
            strictOutOfSync: options.strictOutOfSync,
            dev: options.dev,
        }, targetFiles);
    ...
    try {
        ...
        for (const scanResult of pluginResponse.scanResults) {
            ...
            if (options['print-graph'] && !options['print-deps']) {
                ...
                if (dg) {
                    console.log((0, common_1.depGraphToOutputString)(dg.data.toJSON(), constructProjectName(scanResult)));
                }
            }
            payloads.push({
                method: 'POST',
...
async function assembleLocalPayloads(root, options) {
    ...
    try {
        ...
        const deps = await (0, get_deps_from_plugin_1.getDepsFromPlugin)(root, options);
